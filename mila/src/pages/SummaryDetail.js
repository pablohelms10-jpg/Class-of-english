    ...
    <div>
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--text-light)' }}>
          {activeSummary.text
            ? `${activeSummary.text.split(/\s+/).filter(Boolean).length} palabras`
            : ''
          }
      ...
      {activeMode === 'flashcards' && <FlashcardsMode text={activeSummary.text} />}
      {activeMode === 'quiz' && <QuizMode text={activeSummary.text} />}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16 }}>
        {modes.map(m => (
          <ModeCard key={m.id} mode={m} onClick={() => setActiveMode(m.id)} />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <button
      ...
      style={{
        ...
        background: hovered
          ? 'linear-gradient(135deg, rgba(176,168,164,0.15), rgba(193,183,175,0.1))'
          : 'rgba(255,255,255,0.7)',
